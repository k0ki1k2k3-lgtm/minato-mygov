...
### やること：「minato-mygovフォルダでClaudeを開く」

**ステップ1：エクスプローラーでフォルダを開く**

`C:\Users\Kohta\minato-mygov` のフォルダをエクスプローラーで開く。

**ステップ2：そのフォルダでターミナルを起動する**

フォルダを開いた状態で、上部のアドレスバー（`C:\Users\Kohta\minato-mygov` と表示されているところ）を
**1回クリック**する。するとパス全体が青くハイライトされる。

その状態のまま（パスを消そうとしなくていい）`cmd` と打つ → パスが自動で消えて `cmd` だけになる → Enter。

⚠️ `C:\Users\Kohta\minato-mygovcmd` のようにパスの後ろにくっついてしまった場合は、
クリックが浅くて選択できていない。もう一度しっかりクリックして青くなってから入力する。

```
（青くなった後に cmd と打つと↓こうなる）
cmd     ← Enter を押す

（するとこういう黒い画面が出る）
C:\Users\Kohta\minato-mygov>
```

**ステップ3：Claude を起動する**

黒い画面（コマンドプロンプト）に以下を入力して Enter：

```
claude
```

これだけ。このファイル（CLAUDE.md）が自動で読み込まれ、
プロジェクトの内容を知った状態で会話が始まる。

### なぜフォルダを指定するのか？

Claude Code は「どのフォルダで起動したか」でプロジェクトを判断する。
`C:\Users\Kohta\minato-mygov` で起動すると、このフォルダの `CLAUDE.md` を自動で読む。
別のフォルダで起動するとこのファイルは読まれず、引き継ぎ情報が使えない。

### クローラー側の作業をする場合

同じ手順で `C:\Users\Kohta\minato-mygov-crawler` フォルダを開いて `claude` を起動する。
